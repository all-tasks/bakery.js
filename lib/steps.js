...
async function runSteps(steps = [], context = {}) {
  ...
  async function dispatchStep(nextStepIndex) {
    if (nextStepIndex <= currentStepIndex) throw new Error('next() already called');
    currentStepIndex = nextStepIndex;
    context.steps = {
      after,
      next: async () => dispatchStep(currentStepIndex + 1).catch((error) => {
        throw error;
      }),
    };
    return localSteps[currentStepIndex]?.call(context, context) || null;
  }

  return dispatchStep(0).catch((error) => { throw error; });
}
